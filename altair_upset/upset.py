...
def UpSetAltair(
    ...
    set_to_abbre = pd.DataFrame([[sets[i], abbre[i]] for i in range(len(sets))], 
                               columns=["set", "set_abbre"])
    set_to_order = pd.DataFrame([[sets[i], 1 + sets.index(sets[i])] for i in range(len(sets))], 
    ...
    matrix_view = alt.layer(
        ...
        base.mark_circle(size=glyph_size).transform_filter(
